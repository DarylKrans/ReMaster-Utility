...
namespace V_Max_Tool
{
    ...
    {
        //private static readonly string DISASSM_SPC = "   ";
        private static readonly string[] DISASSM_ABS = new string[] { "BNE", "BEQ", "BMI", "BPL", "BCC", "BCS", "BVC", "BVS", };
        private static readonly HashSet<string> DISASSM_ABS_SET = new HashSet<string>(DISASSM_ABS);
        private static readonly string[] HexBytes = Enumerable.Range(0, 256).Select(i => i.ToString("X2")).ToArray();
        private static readonly char[] HexChars = "0123456789ABCDEF".ToCharArray();

        public class Opcode
        {
            private string _addressingMode;
            public string AddressingMode
            {
                ...
                set
                {
                    ...
                    // Pre-compute everything derived from AddressingMode
                    var last2 = value.Length >= 2 ? value.Substring(value.Length - 2, 2).ToLower() : "";
                    var first3 = value.Length >= 3 ? value.Substring(0, 3) : value;
                    Ext = (last2 == ",x" || last2 == ",y") ? last2 : string.Empty;
                    IsImmediate = first3 == "Imm";
                    IsIndirect = first3 == "Ind";
                }
            }
            ...
            public string Mnemonic
            {
                ...
                {
                    _mnemonic = value;
                    // Pre-compute Absolute check
                    if (value.Length >= 4)
                        Absolute = DISASSM_ABS_SET.Contains(value.Substring(1, 3));
                }
            }
        ...
        private readonly Dictionary<byte, Opcode> Opcodes = new Dictionary<byte, Opcode>
        {
            // --- Official opcodes ---
        ...
        void Disassemble(ref StringBuilder f, byte[] memory, int offset = 0x0000, int endAddress = -1)
        {
            ...
            while (pc < endAddress)
            {
                ...
                if (!Opcodes.TryGetValue(opcodeByte, out var opcode)) opcode = Undefined;
                if (DISASSM_ABS_SET.Contains(opcode.Mnemonic.Substring(1, 3))) opcode.Absolute = true;
                ...
                bool im = opcode.IsImmediate;
                bool ind = opcode.IsIndirect;
                bool par = ind && opcode.Ext.Length > 0;
                char prefix = im ? '#' : '$';

                AppendHex6(ref f, pcoffset);
                f.Append("  ");
                f.Append(HexBytes[opcodeByte]);
                f.Append(' ');
                f.Append(opcode.Length >= 2 ? HexBytes[param1] : b);
                f.Append(' ');
                f.Append(opcode.Length == 3 ? HexBytes[param2] : b);
                f.Append(' ');
                f.Append(opcode.Mnemonic);
                f.Append(' ');
                if (opcode.Absolute) f.Append($"{target:X4}");
                else if (opcode.Length > 1)
                {
                    ...
                    {
                        case 2:
                            f.Append(HexBytes[param1]);
                            break;
                        case 3:
                            f.Append(HexBytes[param2]);
                            f.Append(HexBytes[param1]);
                            break;
                    }
                    if (par) f.Append(')');
        ...
        void AppendHex6(ref StringBuilder sb, int value)
        {
            sb.Append(HexChars[(value >> 20) & 0xF]);
            sb.Append(HexChars[(value >> 16) & 0xF]);
            sb.Append(':');
